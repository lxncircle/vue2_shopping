...
export default {
  ...
  beforeMount() {
    // 复杂的写法
    // 在发请求之前，把接口需要传递的参数进行整理（在服务器发请求之前，把参数整理好，服务器就会返回查询的数据）
  ...
  methods: {
    ...
    // 删除分类的名字
    removeCategoryName() {
      // 把带给服务器的参数置空了，话需要向服务器发请求
      // 带给服务器参数说明可有可无的：
      // 若属性值为空的字符串会把相应字段带给服务器
      // 但把相应的字段变为undefined，当前这个字段不会带给服务器
    ...
    changeOrder(flag) {
      // alert(11)
      // flag形参：标记，代表用户点击综合（1）还是价格（2）【用户点击的时候传递过来的】
